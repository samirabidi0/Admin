...
const MarketPlace = () => {
  ...
  const handleEditClick = (id: number, field: 'name' | 'category' | 'price', value: string | number) => {
    setEditingProductId(id);
    setEditingField(field);
    if (field === 'name') setUpdatedName(value as string);
    if (field === 'category') setUpdatedCategory(value as string);
    if (field === 'price') setUpdatedPrice(value as number);
  };
  ...
  const handleUpdate = async (id: number) => {
    try {
      ...
      await axios.put(`http://127.0.0.1:5000/api/tools/${id}`, updatedFields);
      console.log('Product updated successfully');
              ...
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="py-4 font-semibold text-gray-900 dark:text-white">
                  {editingProductId === product.id && editingField === 'name' ? (
                    ...
                    <span className='flex items-center justify-center gap-2'>
                      <FaRegPenToSquare className='text-green-500 cursor-pointer' onClick={() => handleEditClick(product.id, 'name', product.name)} />
                      {product.name}
                    </span>
                  )}
                </td>
                ...
                <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                  {editingProductId === product.id && editingField === 'category' ? (
                    ...
                    <span className='flex items-center justify-center gap-2'>
                      <FaRegPenToSquare className='text-green-500 cursor-pointer' onClick={() => handleEditClick(product.id, 'category', product.category)} />
                      {product.category}
                    </span>
                  )}
                </td>
                <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                  {editingProductId === product.id && editingField === 'price' ? (
                    ...
                    <span className='flex items-center justify-center gap-2'>
                      <FaRegPenToSquare className='text-green-500 cursor-pointer' onClick={() => handleEditClick(product.id, 'price', product.price)} />
                      {product.price} TND
                    </span>
                  )}
                </td>
                <td className="px-6 py-4">
                  <button className="font-medium flex text-red-600 dark:text-red-500 hover:underline" onClick={() => openModal(product.id)}>
                    <MdDeleteForever size={26} /><span className='mt-1'>Delete</span>
                  </button>
                </td>
              </tr>
            </tbody>
